...
public class FileProcessoratorHandler
{
    ...
    private static void ComputeSomeData(string[] theData, string fileName)
    {
        ...
        stopWatch.Stop();
        Console.WriteLine($"That file took this many {stopWatch.ElapsedMilliseconds} ms");
    }
    ...
    private static void DoSomeSillyMathStuffForSimulationPurposes()
    {
        ...
        var eachs = 0.0;
        foreach (var each in Enumerable.Range(0, 2100))
        {
            eachs += Math.Cbrt(Math.Cos(Math.Pow(42, random.NextDouble())));
        }
    }
    ...
    {
        foreach (var fileName in Directory.GetFiles(TestFileLocation))
        {
